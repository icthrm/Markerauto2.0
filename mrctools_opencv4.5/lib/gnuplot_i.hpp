...
class Gnuplot
{
    ...
    Gnuplot& plot_xy(const X& x, const Y& y, const std::string &title = "" ,const std::string &color = "");
...
//
template<typename X, typename Y>
Gnuplot& Gnuplot::plot_xy(const X& x, const Y& y, const std::string &title , const std::string &color)
{
    ...
    plotfile_xy(name, 1, 2, title , color);
...
//     }
	...
	std::cout<<"Cannot create temporary file \"" << name << "\"";
//         throw GnuplotException(except.str());
...
//         fprintf(stderr, "ErrorMessage: %s\n", strerror(errno)); // strerror: 显示errno对应的错误描述信息
    ...
    {
        std::ostringstream except;
        except << "Cannot open temporary file \"" << name << "\"";
        throw GnuplotException(except.str());
        return "";
    }
